...
**Hover-to-open was removed for good.** Opening Blizzard's breakdown from addon code (`ShowSourceWindow`) writes into its spell-entry pool, whose `iconTexture` is read before it is rewritten (`DamageMeterEntry.lua:54`), so one hover out of combat leaves that pool tainted until `/reload` and the player's own click in the next fight logs a warning per row per meter event - 86 of them in the run that decided it. No combat gate can prevent damage that is done out of combat and shown later. The only clean hover is a frame of our own fed from `C_DamageMeter` (written once in commit `7a46e33`, kept in history); the user chose to drop the feature instead. Left-click stays Blizzard's, right-click is ours.
