...
**Hover came back as a frame of our own.** `Hover.lua` shows a source's spells beside the window in the addon's own panel, fed from `C_DamageMeter.GetCombatSessionSourceFromType` and painted through `C_Spell.GetSpellName` / `GetSpellTexture`, `AbbreviateNumbers`, `SetText` and `SetTexture`, all `AllowedWhenTainted`. The row's own GUID is Secret in combat, so the source is resolved the way EnhanceQoL does it: `isLocalPlayer` (NeverSecret) to `UnitGUID("player")`, otherwise the row's `classFilename` (NeverSecret) to the party member of that class when the class is unique in the party. Blizzard's source window is never touched. Blizzard's own breakdown, opened by their click, takes priority while shown.

**A right-click on the bars that opens Blizzard's menu was tried and does not exist.** Their dropdowns open in `OnMouseDown` (`MenuTemplates.xml:46,77`); a secure action button can synthesise only a click, which runs `OnClick` and does nothing there, and opening the menu from our code builds it under our taint. What survives is `Menu.ModifyMenu` on the header's type menu, which now carries Lock, Hide, Reset and settings.
