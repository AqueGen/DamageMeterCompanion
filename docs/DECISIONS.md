...
**Any `hooksecurefunc` on a function Blizzard calls while rendering the list carries our taint into their Secret comparisons**, whatever the hook body does. Both `InitEntry` (Hover, ContextMenu) and `UpdateValue` (the first Format) produced `attempt to compare field 'sourceDisplayType' (a secret number value, while execution tainted by ...)` in combat, each confirmed in game by disabling one at a time. Nothing hooks the render path any more: Format paints and Entries attaches its handlers from one shared timer (`ns.OnSweep`), so our code is never on Blizzard's stack. Entries re-registers each bar for the left button only, so Blizzard's OnClick keeps the left click and a right-click can be ours, and hooks OnEnter, OnLeave and OnMouseDown once per frame - scripts Blizzard never sets on an entry.
...
**Left-click is Blizzard's, right-click is ours, and hover works only while the row's GUID is readable.** `C_DamageMeter.GetCombatSessionSourceFromType` is documented `SecretArguments = "AllowedWhenUntainted"`, and `ShowSourceWindow` passes the row's `sourceGUID` to it. From our stack that is a hard error whenever the GUID is Secret - in combat, and after combat until the data is fetched again. So the breakdown is opened by Blizzard's own OnClick (untainted, works in combat, Shift-click pins), the bars keep the left button registered and lose the right one so a right-click reaches our menu, and hover checks `issecretvalue` on the GUID and waits when it is. A `Refresh()` of our own after combat was tried to bring hover back sooner and dropped the same evening: one frame after `PLAYER_REGEN_ENABLED` the fetch still returned Secret values (the session closes on the server, not the client - Details reads `issecretvalue` as its in-combat signal for exactly that reason), and Blizzard's `InitEntry` ran inside our call, so the OnClick closures it installed were tainted and their own click warned out of combat until their next refresh replaced them. **A `Refresh()` from addon code is never safe**: every closure and field it produces is tainted until Blizzard rewrites it. Hover waits for Blizzard's own fetch. An unpinned breakdown is closed as combat starts, because its `sourceGUID` was written from our stack and their refresh compares it against every Secret row.
...
- An unpinned breakdown closes as combat starts (`Entries.CloseHoverBreakdowns`). The alternative was a warning per meter event for as long as it stayed open.
