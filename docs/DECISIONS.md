...
- `Entries.RefreshAfterCombat` closes an open breakdown after every pull if it holds a Secret GUID, which in practice is every breakdown opened during combat. The alternative was a warning per row on the refresh that makes hover work again.
